...
//   };
  ...
  // const fetchRecommendedSong = async (moodId) => {
  //   try {
  //     const token = getToken();
  //     const response = await axios.get(`/api/mood-recommendations/${moodId}/`, {
  //       headers: {
  //         Authorization: `Bearer ${token}`,
  //       },
  //     });
  //     return response.data[0]; // Assuming the API returns an array of songs
  //   } catch (error) {
  ...
  const fetchMoodEntries = async () => {
    try {
      ...
      const entriesWithMoodNames = await Promise.all(
        response.data.map(async (entry) => {
          const moodName = await fetchMoodName(entry.mood);
          const recommendedSong = await fetchRecommendedSong(entry.mood);
          return { ...entry, moodName, recommendedSong };
        })
      );
      setMoodEntries(entriesWithMoodNames);
    } catch (error) {
            ...
            {moodEntries.map((entry) => (
              <li key={entry.id} className="mood-entry-item">
                <div className="mood-entry-details">
                  ...
                  {entry.recommendedSong && (
                    <p className="mood-entry-song">
                      Recommended Song: {entry.recommendedSong.title} by{' '}
                      {entry.recommendedSong.artist}
                    </p>
                  )}
                </div>
              </li>
